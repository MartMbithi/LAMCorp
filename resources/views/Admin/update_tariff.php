<?php
    ...
    if(isset($_POST['updateTariff']))
    {
        ...
        */
                ...
                $query="UPDATE LAMCorp_waterTariffs SET  t_name = ?, cost_per_litre = ? WHERE t_code =?";
                $stmt = $mysqli->prepare($query);
                //bind paramaters
                $rc=$stmt->bind_param('sss', $t_name, $cost_per_litre, $tariff);
                $stmt->execute();

                //declare a varible which will be passed to alert function
                if($stmt)
                {
                 $success = "Water Tariff Updated " && header("refresh:1; url=manage_watertariff.php");
                }
                else 
                {
...
<body data-spy="scroll" data-target="#navSection" data-offset="100">
    ...
    <?php
        $tariff = $_GET['tariff'];
        $ret="SELECT * FROM  LAMCorp_waterTariffs WHERE t_code = ? "; 
                        ...
                        <div class="page-header">
                            ...
                            <nav class="breadcrumb-one" aria-label="breadcrumb">
                                <ol class="breadcrumb">
                                    ...
                                    <li class="breadcrumb-item"><a href="javascript:void(0);">Manage Tariffs</a></li>
                                    <li class="breadcrumb-item active" aria-current="page"><span>Update <?php echo $row->t_code;?> Tariff</span></li>
                                </ol>
                            </nav>
                                        ...
                                        <form method="post" enctype="multipart/form-data" >
                                            <div class="form-row mb-4">
                                                <div class="form-group col-md-6">
                                                    ...
                                                    <input required type="text" readonly name="t_code" value="<?php echo $row->t_code;?>" class="form-control">
                                                </div>
                                                <div class="form-group col-md-6">
                                                    <label for="inputPassword4">Tariff Name</label>
                                                    <input required type="text" value="<?php echo $row->t_name;?>"  name="t_name"  class="form-control">
                                                </div>
                                            </div>
                                            <div class="form-group mb-4">
                                                <label for="inputAddress">Tariff Water Price Per Litre in Ksh</label>
                                                <textarea type="text" required name="cost_per_litre"  class="form-control" rows="5" ><?php echo $row->cost_per_litre;?></textarea>
                                            </div>
                                        <button type="submit" name="updateTariff" class="btn btn-primary mt-3">Update Water Tariff Details</button>
                                        </form>
