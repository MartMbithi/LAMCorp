<?php
    ...
    if(isset($_POST['updateWaterKiosk']))
    {
        /*
            ...
            {
        */
                $waterpoint_number = $_SESSION['wp_number'];
                ...
                $query="UPDATE  LAMCorp_waterPoints SET wp_phone = ?, wp_location=?, wp_opening_hrs=?, wp_closing_hrs=?, wp_status=? WHERE wp_number =?";
                $stmt = $mysqli->prepare($query);
                //bind paramaters
                $rc=$stmt->bind_param('ssssss', $wp_phone, $wp_location, $wp_opening_hrs, $wp_closing_hrs, $wp_status, $waterpoint_number);
                $stmt->execute();

                //declare a varible which will be passed to alert function
                if($stmt)
                {
                 $success = "Water Point Updated" && header("refresh:1; url=manage_waterpoint.php");
                }
                else 
                {
                        ...
                        <div class="page-header">
                            ...
                            <nav class="breadcrumb-one" aria-label="breadcrumb">
                                <ol class="breadcrumb">
                                    ...
                                    <li class="breadcrumb-item active" aria-current="page"><span>Update <?php echo $row->wp_number;?></span></li>
                                </ol>
                            </nav>
                                    ...
                                    <div class="widget-content widget-content-area">
                                        <form method="post" enctype="multipart/form-data" >
                                            ...
                                            <div class="form-group mb-4">
                                                ...
                                                <textarea type="text" required name="wp_location" class="form-control" rows="5" ><?php echo $row->wp_location;?></textarea>
                                            </div>
                                            <div class="form-row">
                                                <div class="form-group col-md-6">
                                                    <label for="inputAddress">Water Point | Kiosk Opening  Hours</label>
                                                    <input required type="time"  name="wp_opening_hrs" value="<?php echo $row->wp_opening_hrs;?>"  class="form-control">
                                                </div>
                                                <div class="form-group col-md-6">
                                                    <label for="inputAddress">Water Point | Kiosk Closing Hours</label>
                                                    <input required type="time"  name="wp_closing_hrs" value="<?php echo $row->wp_closing_hrs;?>"  class="form-control">
                                                </div>
                                            </div>
                                            <div class="form-row">
                                        ...
                                        <button type="submit" name="updateWaterKiosk" class="btn btn-primary mt-3">Update Water Point Details</button>
                                        </form>
                                    </div>
                                </div>
                            </div>
